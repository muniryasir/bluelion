...
var stepMarkersRed = [];
var pin_color;
var stepinfowindowsBlue = [];
var stepinfowindowsRed = [];
var globalContext;
var globalMap;
    ...
    data: {'context':context},
      ...
      for(var i=0; i<partsmain.length-1; i++) {
        ...
        <div class="map_nav" style="float:right;width:16%;height:50px;line-height:60px;display: flex;flex-wrap: wrap;justify-content: center;text-align: center;align-items: center;">
          <a href="javascript:;" onclick="showUserPosition()" style="display: inline-block;margin-top:10px;">
      ...
      if (pin_color == "BLUE") {
        stepinfowindowsBlue.push(infowindow);
      } else if (pin_color == "RED") {
        stepinfowindowsRed.push(infowindow);
      }
      ...
      label: {
        ...
        if (pin_color == "BLUE") {
          stepMarkersBlue.push(markerstep);
        } else if (pin_color == "RED") {
...
function openNativeMap(lat, longt) {
  if /* if we're on iOS, open in Apple Maps */
    ...
    window.open(`maps://maps.google.com/maps?daddr=<${lat}>,<${longt}>&amp;ll=`);
else /* else use Google */
    window.open(`https://maps.google.com/maps?daddr=<${lat}>,<${longt}>&amp;ll=`);
}